...
const fragmentShader = `
  ...
  void mainImage(out vec4 fragColor, in vec2 fragCoord) {
      ...
      vec3 baseColor = vec3(0.24, 0.88, 0.81);
	...
	<Canvas dpr={[1, 1.5]} performance={{ min: 0.5 }}>
			...
			<div className="relative z-10 flex flex-col items-center text-center px-6">
				<div ref={badgeWrapperRef}>
					<Badge className="mb-6 bg-[#3ee0cf]/10 hover:bg-[#3ee0cf]/15 text-[#3ee0cf] backdrop-blur-md border border-[#3ee0cf]/20 uppercase tracking-wider font-medium flex items-center gap-2 px-4 py-1.5">
						<span className="text-[10px] font-light tracking-[0.18em] text-[#3ee0cf]/80">
							{badgeLabel}
						</span>
						<span className="h-1 w-1 rounded-full bg-[#3ee0cf]/60" />
						<span className="text-xs font-light tracking-tight text-[#3ee0cf]">
							{badgeText}
						</span>
					</Badge>
				</div>
				...
				<h1
					ref={headingRef}
					className="text-5xl md:text-7xl max-w-4xl font-bold tracking-tight text-foreground mb-4"
				>
					{title}
				</h1>
				...
				>
					{ctaButtons.map((button, index) => {
						...
						const classes = isPrimary
							? "px-8 py-3 rounded-xl text-base font-medium backdrop-blur-lg bg-[#3ee0cf] hover:bg-[#3ee0cf]/90 text-black shadow-lg transition-all cursor-pointer"
							: "px-8 py-3 rounded-xl text-base font-medium border-border text-foreground hover:bg-accent backdrop-blur-lg transition-all cursor-pointer";
				...
				{microDetails.length > 0 && (
					<ul
						ref={microRef}
						className="mt-8 flex flex-wrap justify-center gap-6 text-xs tracking-tight text-muted-foreground"
					>
